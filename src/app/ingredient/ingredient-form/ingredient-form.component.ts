...
export const createSearchResultForm = (searchResult: SearchResult) =>
  new FormGroup<SearchResultForm>({
    id: new FormControl<number | undefined>(searchResult.id, {
      nonNullable: true,
    }),
    name: new FormControl<string>(searchResult.name, { nonNullable: true }),
  });

export const createIngredientForm = (ingredient: Ingredient) =>
  new FormGroup<IngredientForm>({
    id: new FormControl<number | undefined>(ingredient.id, {
      nonNullable: true,
    }),
...
export class IngredientFormComponent extends ItemFormComponentAbstract<
  Ingredient,
  IngredientPatch
> {
  readonly form: FormGroup<IngredientDetailsForm> =
    new FormGroup<IngredientDetailsForm>({
  ...
  readonly categoryOptions: number[] = [
